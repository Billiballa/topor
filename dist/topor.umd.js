(function (global, factory) {
  ...
  (global = global || self, factory(global.Topor = {}));
  ...
  Animatable.prototype = {
    ...
    animate: function (path, loop) {
      ...
      if (!target) {
        log_1('Property "' + path + '" is not existed in element ' + el.id);
        return;
      }
  ...
  function getBoxPosition(blockHeiht, style, rect) {
    ...
    if (rect) {
      ...
      if (textPosition instanceof Array) {
        ...
        var res = text.adjustTextPositionOnRect(textPosition, rect, style.textDistance);
        baseX = res.x;
  ...
  Painter.prototype = {
    ...
    insertLayer: function (zlevel, layer) {
      ...
      if (layersMap[zlevel]) {
        log_1('ZLevel ' + zlevel + ' has been used already');
        return;
      } // Check if is a valid layer


      if (!isLayerValid(layer)) {
        log_1('Layer of zlevel ' + zlevel + ' is not valid');
        return;
      }
    ...
    _updateLayerStatus: function (list) {
      ...
      for (var i = 0; i < list.length; i++) {
        ...
        if (!layer.__builtin__) {
          log_1('ZLevel ' + zlevel + ' has been used by unkown layer ' + layer.id);
        }
  ...
  var domHandlers = {
    ...
    touchstart: function (event) {
      ...
      event = normalizeEvent$1(this.dom, event); // Mark touch, which is useful in distinguish touch and
  ...
  function initDomHandler(instance) {
    ...
    util.each(pointerHandlerNames, function (name) {
      instance._handlers[name] = util.bind(domHandlers[name], instance);
    });
  ...
  */
  ...
  var ZRender = function (id, dom, opts) {
    ...
    this.storage = storage;
    this.painter = painter;
    var handerProxy = !env_1.node && !env_1.worker ? new HandlerProxy(painter.getViewportRoot()) : null;
    ...
    for (var i = 0; i < dataLength;) {
      ...
      switch (cmd) {
        ...
        case CMD$3.A:
          ...
          var isCircle = isAroundZero$1(dThetaPositive - PI2$4) && !isAroundZero$1(dThetaPositive);
          var large = false;
  ...
  var svgTextDrawRectText = function (el, rect, textRect) {
    ...
    var font = style.font || [style.fontStyle || '', style.fontWeight || '', style.fontSize || '', style.fontFamily || ''].join(' ') || text.DEFAULT_FONT;
    var verticalAlign = getVerticalAlignForSvg(style.textVerticalAlign);
    textRect = text.getBoundingRect(text$2, font, align, verticalAlign, style.textPadding, style.textLineHeight);
    ...
    if (textPosition instanceof Array) {
      ...
      var newPos = text.adjustTextPositionOnRect(textPosition, rect, distance);
      x = newPos.x;
      y = newPos.y;
    ...
    if (el.__text !== text$2 || el.__textFont !== font) {
      ...
      for (var i = 0; i < nTextLines; i++) {
        ...
        if (!tspan) {
          tspan = tspanList[i] = createElement$1('tspan');
  ...
  ShadowManager.prototype.update = function (svgElement, displayable) {
    ...
    if (hasShadow(style)) {
      var that = this;
      Definable_1.prototype.update.call(this, displayable, function (style) {
  ...
  ShadowManager.prototype.markUsed = function (displayable) {
    ...
    if (style && style._shadowDom) {
      Definable_1.prototype.markUsed.call(this, style._shadowDom);
    }
  };
  ...
  function createMethodNotSupport(method) {
    return function () {
      log_1('In SVG mode painter not support method "' + method + '"');
    };
  } // Unsuppoted methods
  ...
  if (!env_1.canvasSupported) {
    ...
    var tmpRect$3 = new BoundingRect_1();
    ...
    var drawRectText = function (vmlRoot, rect, textRect, fromTextEl) {
      ...
      if (!fromTextEl) {
        ...
        if (textPosition instanceof Array) {
          ...
          var res = text.adjustTextPositionOnRect(textPosition, rect, distance);
          x = res.x;
  ...
  function createMethodNotSupport$1(method) {
    return function () {
      log_1('In IE8.0 VML mode painter not support method "' + method + '"');
    };
  } // Unsupported methods
  ...
  function getEdgePosition(sourceNode, targetNode) {
    var sourceRect = getVertexOfNode(sourceNode);
    var targetRect = getVertexOfNode(targetNode);
    var source = getIntrOfRectAndLine(sourceRect.points, [sourceRect, targetRect]);
    var target = getIntrOfRectAndLine(targetRect.points, [sourceRect, targetRect]);
  ...
  function getIntrOfRectAndLine(rectPoints, linePoints) {
    var point;

    for (var i = 0; i < rectPoints.length; i++) {
      point = getIntrOfTwoLine(rectPoints[i], rectPoints[i + 1 === rectPoints.length ? 0 : i + 1], linePoints[0], linePoints[1]);
  ...
  function getIntrOfTwoLine(a, b, c, d) {
    // 三角形abc 面积的2倍 
    var area_abc = (a.x - c.x) * (b.y - c.y) - (a.y - c.y) * (b.x - c.x); // 三角形abd 面积的2倍 

    var area_abd = (a.x - d.x) * (b.y - d.y) - (a.y - d.y) * (b.x - d.x); // 面积符号相同则两点在线段同侧,不相交; 
    ...
    var area_cda = (c.x - a.x) * (d.y - a.y) - (c.y - a.y) * (d.x - a.x); // 三角形cdb 面积的2倍（通过另外三个加减即可得出）
  ...
  function getLongestLine(points) {
    var source,
        target,
        length = 0;
    points.reduce(function (newSource, newTarget) {
      var newLength = getLineLength(newSource, newTarget);
  ...
  function getLabelPositionOfEdge(edge, label) {
    var source = edge.source;
    var target = edge.target; // 若连线是从右侧指向左侧 则交换起始点后计算
    ...
    var rotate = Math.atan2(target.y - source.y, target.x - source.x); // 连线的中心点

    var midP = {
      x: (target.x + source.x) / 2,
      y: (target.y + source.y) / 2 // 比例

    };
    var lpb = label.x / 2 / edge.length;
    var x = midP.x - (target.x - source.x) * lpb + label.y * Math.sin(rotate);
    var y = midP.y - (target.y - source.y) * lpb - label.y * Math.cos(rotate);
    return {
      x: x,
      y: y,
      rotate: -rotate // zrender的旋转方向和原声canvas相反

    };
  }
  var shape = {
    getEdgePosition: getEdgePosition,
    getVertexOfNode: getVertexOfNode,
    getIntrOfRectAndLine: getIntrOfRectAndLine,
    getIntrOfTwoLine: getIntrOfTwoLine,
    getLineLength: getLineLength,
    getLongestLine: getLongestLine,
    getLabelPositionOfEdge: getLabelPositionOfEdge
  };

  var util$3 = _objectSpread2({}, zrender$2.util, {}, shape, {
    /**
     * @func
     * @desc Mixin模式
  ...
  function () {
    ...
    _createClass(StateMixin, [{
      ...
      value: function getOption() {
        return util$3.clone(this._globalOption);
      }
    }, {
      key: "setOption",
      value: function setOption(newOption) {
        util$3.merge(this._globalOption, newOption, true);
        return this.getOption();
      }
    }, {
      key: "registerPlugin",
      value: function registerPlugin(plugin, option) {
      ...
      key: "registerElement",
      value: function registerElement(name, element) {
      ...
      value: function getElAttr(name) {
        ...
        return util$3.clone(el.attr);
      }
    }, {
      key: "setElAttr",
      value: function setElAttr(name, newAttr) {
        var el = this._elements[name];
        ...
        util$3.merge(el.attr, newAttr, true);
        return this.getElAttr(name);
      }
    }, {
      key: "addData",
      value: function addData(data) {
        var elClass = this._elements[data.type];

        if (!elClass) {
          return;
        }

        var el = new elClass.handle(this, util$3.clone(data));
      ...
      key: "setData",
      value: function setData(id, newData) {
        ...
        data.update(util$3.clone(newData));
        return data;
      }
    }, {
      key: "deleteData",
      value: function deleteData(dataId) {
        if (dataId == null) {
          for (var _dataId in this._data) {
            this._data[_dataId].dispose();
          }
  ...
  function (_EventMixin) {
    _inherits(BaseElement, _EventMixin);

    function BaseElement(graph, data) {
      var _this;

      _classCallCheck(this, BaseElement);

      _this = _possibleConstructorReturn(this, _getPrototypeOf(BaseElement).call(this));
      _this.graph = graph;
      _this.root = graph.root;
      _this.data = data;
      _this.id = data.id;
      return _this;
    }
    ...
    _createClass(BaseElement, [{
      ...
      key: "getFinalAttr",
      value: function getFinalAttr() {
        this.finalAttr = this.graph.getElAttr(this.type);
        util$3.merge(this.finalAttr, this.data.attr, true);
      }
    }, {
      key: "setAttr",
      value: function setAttr(newAttr) {
        util$3.merge(this.data.attr, newAttr, true);
        this.render();
      }
    }, {
      key: "setMode",
  ...
  function (_BaseElement) {
    ...
    function NodeElement(graph, data) {
      var _this;

      _classCallCheck(this, NodeElement);

      _this = _possibleConstructorReturn(this, _getPrototypeOf(NodeElement).call(this, graph, data));
      _this.type = 'node';
      return _this;
    } // 注册时调用
    ...
    _createClass(NodeElement, [{
      key: "init",
      value: function init() {
        this.image = new zrender$2.Image();
        this.label = new zrender$2.Text({
          silent: true
        });
        this.group = new zrender$2.Group();
      ...
      value: function render() {
        ...
        this.label.attr({
          style: {
            ...
            x: 0,
            y: function () {
              switch (_this2.finalAttr.labelPosition) {
                case 'top':
                  return -(_this2.finalAttr.imageSize / 2 + _this2.finalAttr.labelFontSize * 2);

                case 'middle':
                  return -_this2.finalAttr.labelFontSize;

                case 'bottom':
                default:
                  return _this2.finalAttr.imageSize / 2;
              }
      ...
      key: "install",
      value: function install(graph) {
        this.addEvent(graph);
      }
    }, {
      key: "addEvent",
      value: function addEvent(graph) {
        var lastPosition; // 初始点击位置

        var moveNode = null; // 被点击的节点

        graph.zrender.on('mousedown', function (e) {
          // 点击到节点 则开始拖拽
          if (e.target && e.target.parent && e.target.parent.toporType === 'node' && e.target.toporType !== "node-label") {
            moveNode = e.target.parent;
            lastPosition = [e.event.zrX, e.event.zrY];
          }
        });
        graph.zrender.on('mouseup', function () {
          moveNode = null;
        }); // 防止鼠标移出画布后依然可以拖拽
        ...
        graph.zrender.on('mousemove', function (e) {
          if (moveNode) {
            var scale = graph.root.scale[0];
            var newPX = moveNode.position[0] + (e.event.zrX - lastPosition[0]) / scale,
                newPY = moveNode.position[1] + (e.event.zrY - lastPosition[1]) / scale; // 触发更新

            moveNode.toporEl.setAttr({
              x: newPX,
              y: newPY
  ...
  /*#__PURE__*/
  function (_BaseElement) {
    _inherits(EdgeElement, _BaseElement);

    function EdgeElement(graph, data) {
      var _this;

      _classCallCheck(this, EdgeElement);

      _this = _possibleConstructorReturn(this, _getPrototypeOf(EdgeElement).call(this, graph, data));
      _this.type = 'edge';
      return _this;
    }

    _createClass(EdgeElement, [{
      key: "init",
      value: function init() {
        this.line = new zrender$2.Line();
        this.label = new zrender$2.Text({
          silent: true
        });
        this.group = new zrender$2.Group();
      ...
      value: function render() {
        ...
        var sourceElement = this.graph.findElementById(this.finalAttr.source);
        var targetElement = this.graph.findElementById(this.finalAttr.target);

        var handleRefresh = function handleRefresh() {
          var sourceAttrs = sourceElement.finalAttr;
          var targetAttrs = targetElement.finalAttr;

          _this2.refreshEdgePosition(sourceAttrs, targetAttrs);
        }; // 连线随节点移动
      ...
      key: "refreshEdgePosition",
      value: function refreshEdgePosition(sourceNode, targetNode) {
        var linePoints = util$3.getEdgePosition(sourceNode, targetNode);

        if (linePoints) {
          var source = linePoints.source,
              target = linePoints.target;
          this.line.attr({
            shape: {
              x1: source.x,
              y1: source.y,
          ...
          if (!this.finalAttr.labelHide) {
            var labelPosition = util$3.getLabelPositionOfEdge(util$3.getLongestLine([source, target]), {
              x: 0,
              y: 10
            });
            this.label.attr({
  ...
  /*#__PURE__*/
  function (_util$mix) {
    _inherits(BaseGraph, _util$mix);

    function BaseGraph(dom, option) {
      var _this;

      _classCallCheck(this, BaseGraph);

      _this = _possibleConstructorReturn(this, _getPrototypeOf(BaseGraph).call(this));
      _this.dom = dom;
      _this.zrender = zrender$2.init(dom, option);
      _this.root = new zrender$2.Group();

      _this.zrender.add(_this.root);

      initElements(_assertThisInitialized(_this));
      initPlugins(_assertThisInitialized(_this));
      return _this;
    }
    /**
     * set data并开始绘制元素
    ...
    _createClass(BaseGraph, [{
      ...
      value: function render(data) {
        ...
        var _loop = function _loop(type) {
          if (Object.prototype.hasOwnProperty.call(data, type)) {
            ...
            _this2.setElAttr(type, els.attr);

            els.data.forEach(function (el) {
              _this2.addData(_objectSpread2({
                type: type
              }, el));
            });
          }
      ...
      key: "findDataById",
      value: function findDataById(id) {
        var el = this.findElementById(id);
    ...
    return BaseGraph;
  }(util$3.mix(StateMixin, EventMixin));

  var CustomGraph =
  /*#__PURE__*/
  function (_BaseGraph) {
    _inherits(CustomGraph, _BaseGraph);

    function CustomGraph(dom, option) {
      var _this;

      _classCallCheck(this, CustomGraph);

      _this = _possibleConstructorReturn(this, _getPrototypeOf(CustomGraph).call(this, dom, option));
    ...
    _createClass(CustomGraph, [{
      ...
      key: "refreshLineIndexs",
      value: function refreshLineIndexs(els) {
        var countMap = {};
        var _iteratorNormalCompletion = true;
        var _didIteratorError = false;
        var _iteratorError = undefined;
        ...
        try {
          for (var _iterator = els[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
            ...
            if (el.type === 'line') {
              var _key = "".concat(el.source, "~").concat(el.target);

              if (!countMap[_key]) {
                countMap[_key] = [];
              }

              countMap[_key].push(el);
            }
          }
        } catch (err) {
          _didIteratorError = true;
      ...
      value: function addDragEvent() {
        ...
        var moveStatus = false; // 是否可以拖动

        var lastPosition; // 初始点击位置

        this.zrender.on('mousedown', function (e) {
          //没有点击到任何图形 则开始整体拖拽
          if (!e.target) {
            moveStatus = true;
            lastPosition = [e.event.zrX, e.event.zrY];
        ...
        this.zrender.on('mouseup', function () {
          moveStatus = false;
        }); // 防止鼠标移出画布后依然可以拖拽

        this.zrender.on('mouseout', function () {
          moveStatus = false;
        });
        this.zrender.on('mousemove', function (e) {
          if (moveStatus) {
            var PX = _this2.root.position[0] + e.event.zrX - lastPosition[0],
                PY = _this2.root.position[1] + e.event.zrY - lastPosition[1];

            _this2.root.attr('position', _this2.filterMoveBoundary(_this2, [PX, PY]));
      ...
      key: "addZoomEvent",
      value: function addZoomEvent() {
        var _this3 = this;

        var scale = 1.0;
        this.zrender.on('mousewheel', function (e) {
          var _ref = [e.event.zrX, e.event.zrY],
              ex = _ref[0],
              ey = _ref[1];

          var _this3$root$position = _slicedToArray(_this3.root.position, 2),
              px = _this3$root$position[0],
              py = _this3$root$position[1];

          var _this3$root$scale = _slicedToArray(_this3.root.scale, 2),
              sx = _this3$root$scale[0],
              sy = _this3$root$scale[1];

          var newScale = scale + e.wheelDelta / 10.0; // 控制缩放范围

          if (newScale < 0.2 || newScale > 5) return;
          scale = newScale;

          _this3.root.animateTo({
            scale: [scale, scale],
            position: _this3.filterMoveBoundary(_this3, [ex + (px - ex) * (scale / sx), ey + (py - ey) * (scale / sy)])
          }, 180);
        });
      } // 根据移动边界，计算新的position，防止内容移动到离画布过远的位置

    }, {
      key: "filterMoveBoundary",
      value: function filterMoveBoundary(graph, _ref2) {
        var _ref3 = _slicedToArray(_ref2, 2),
            PX = _ref3[0],
            PY = _ref3[1];

        var MOVE_PADDING = 100; // 边界大小

        var _ref4 = [graph.zrender.getWidth(), graph.zrender.getHeight()],
            ZRW = _ref4[0],
            ZRH = _ref4[1]; // 画布大小

        var RRect = graph.root.getBoundingRect(); // root盒

        var _ref5 = [RRect.x, RRect.y, RRect.width, RRect.height],
            RX = _ref5[0],
            RY = _ref5[1],
            RW = _ref5[2],
            RH = _ref5[3];
        var RS = graph.root.scale[0]; // root缩放

        var safePX = PX,
            safePY = PY; // 最大

        if (RX * RS + PX > ZRW - MOVE_PADDING) {
          safePX = ZRW - MOVE_PADDING - RX * RS;
        }

        if (RY * RS + PY > ZRH - MOVE_PADDING) {
          safePY = ZRH - MOVE_PADDING - RY * RS;
        } // 最小


        if ((RX + RW) * RS + PX < MOVE_PADDING) {
          safePX = MOVE_PADDING - (RX + RW) * RS;
        }

        if ((RY + RH) * RS + PY < MOVE_PADDING) {
          safePY = MOVE_PADDING - (RY + RH) * RS;
        }
  ...
  exports.CustomGraph = CustomGraph;
